    ...
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <div className="flex border-b border-gray-200">
        {[
          ...
          <button
            key={id}
            onClick={() = aria-label="Icon button"> setActiveTab(id as 'overview' | 'devices' | 'alerts')}
            ...
            device.alerts.filter(alert => !alert.resolved).map(alert => (
              <div key={alert.id} className={`p-4 border rounded-lg ${getAlertSeverityColor(alert.severity)}`}>
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={() = aria-label="Icon button"> resolveAlert(device.id, alert.id)}
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() = aria-label="Icon button"> setSelectedDevice(null)}
